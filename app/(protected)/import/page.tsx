...
// Progress component
function ImportProgressIndicator({ progress }: { progress: ImportProgress | null }) {
  if (!progress) return null;

  const progressPercent = Math.round((progress.currentStep / progress.totalSteps) * 100);
  const itemsPercent = progress.itemsTotal > 0 ? Math.round((progress.itemsProcessed / progress.itemsTotal) * 100) : 0;
    ...
    <div className="space-y-4">
      ...
      {progress.itemsTotal > 0 && (
        <div className="text-xs text-gray-500">
          Items: {progress.itemsProcessed} / {progress.itemsTotal} ({itemsPercent}%)
        </div>
      )}
    ...
    <div className="space-y-4">
      <div className={`p-4 rounded-lg ${result.success ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
        ...
        <div className="mt-2 space-y-1 text-sm">
          ...
          {result.summary.skipped > 0 && <div>Items skipped: {result.summary.skipped}</div>}
          {result.summary.errors > 0 && <div className="text-red-600">Errors: {result.summary.errors}</div>}
        </div>
      </div>
...
export default function ImportPage() {
    ...
    try {
      ...
      if (importType === "bitwarden") {
        ...
        if (!validateBitwardenExport(data)) {
          throw new Error("Invalid Bitwarden export format. Please export your vault as JSON from Bitwarden.");
        }
      }
      ...
      if (error instanceof SyntaxError) {
        throw new Error("Invalid JSON file. Please ensure you've exported the correct format.");
      }
      throw error;
    }
  };
  ...
  const handleImport = async () => {
    if (!user) {
      setResult({
        success: false,
        summary: { foldersCreated: 0, credentialsCreated: 0, totpSecretsCreated: 0, errors: 1, skipped: 0 },
    ...
    try {
      ...
      if (importType === "bitwarden") {
        ...
        const importResult = await importServiceRef.current.importBitwardenData(fileContent, user.$id);
  ...
  const isFileValid = file && (
    ...
    (!["bitwarden", "json"].includes(importType) && file.name.endsWith('.csv'))
  );
  ...
  return (
    <div className="max-w-4xl mx-auto py-10">
      <div className="mb-8">
        ...
        <p className="text-gray-600">Import your passwords and data from other password managers</p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        <div className="space-y-6">
          <Card className="p-6">
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-medium mb-3">Select Password Manager</label>
              ...
              {importType === "bitwarden" && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <h3 className="font-medium text-blue-800 mb-2">How to export from Bitwarden:</h3>
                  <ol className="text-sm text-blue-700 space-y-1 list-decimal list-inside">
                    <li>Log into your Bitwarden web vault</li>
                    <li>Go to <strong>Tools</strong> → <strong>Export Vault</strong></li>
                    <li>Select <strong>JSON (.json)</strong> format</li>
                    <li>Enter your master password and click <strong>Export Vault</strong></li>
              ...
              <div>
                <label className="block text-sm font-medium mb-3">
                  Select File 
                  {importType === "bitwarden" && <span className="text-gray-500">(JSON format)</span>}
                </label>
                <Input
                  type="file"
          ...
          {!importing && !result && (
            <Card className="p-6">
              ...
              <div className="space-y-2 text-sm text-gray-600">
                <p>• <strong>Do not navigate away</strong> from this page during import</p>
            ...
            <div className="space-y-3 text-sm">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="text-gray-600">Usernames, passwords, URLs, and notes</div>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-2 h-2 bg-green-500 rounded-full mt-2 flex-shrink-0"></div>
                <div>
                  ...
                  <div className="text-gray-600">Two-factor authentication codes (automatically separated)</div>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-2 h-2 bg-green-500 rounded-full mt-2 flex-shrink-0"></div>
                <div>
                  ...
                  <div className="text-gray-600">Folder structure and item organization</div>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-2 h-2 bg-green-500 rounded-full mt-2 flex-shrink-0"></div>
                <div>
                  ...
                  <div className="text-gray-600">Additional fields and metadata</div>
                </div>
              </div>
            </div>
          </Card>
